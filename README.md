...
| 🛒 Plan ▸ Backlog | A scored backlog of the items held back from a launch, totalling each priority band in weeks rather than rows, and splitting the committed band by product | `content/backlog/` |
...
| `content/backlog/` | One markdown table per backlog board — `src/backlog-config.ts` picks which |
